...
test('family care plan renders equivalent page-title on both apps', async ({ browser, nextjs, spa }) => {
  ...
  await pageNext.goto(`${nextjs.url}/family/care-plan`);
  ...
  await pageSpa.goto(`${spa.url}/family/care-plan`);
  ...
  // cps-spa-only: either care plan items or empty state must render
  const hasItems = await pageSpa.locator('[data-testid="care-plan-item"]').count();
  const hasEmpty = await pageSpa.locator('[data-testid="care-plan-empty"]').count();
