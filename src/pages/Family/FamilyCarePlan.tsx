...
export function FamilyCarePlan() {
  ...
  useEffect(() => {
    ...
    familyApi
      ...
      .catch(() => setError(t('family.carePlan.loadFailed')));
  }, [patientId, t]);
          ...
          >
            <div className="mb-3 flex items-center gap-3">
              ...
              <span className="text-sm text-slate-500">
                {t('family.carePlan.effective', {
                  date: new Date(plan.effectiveDate).toLocaleDateString(dateLocale),
                })}
              </span>
              {plan.reviewDate && (
                <>
                  <span className="text-slate-300">·</span>
                  <span className="text-sm text-slate-500">
                    {t('family.carePlan.nextReview', {
                      date: new Date(plan.reviewDate).toLocaleDateString(dateLocale),
                    })}
                  </span>
                </>
              )}
